...
export function SiteHeader({ variant: _variant = "home" }: SiteHeaderProps) {
  ...
  useEffect(() => {
    ...
    async function syncUser() {
      ...
      if (!active) return;
      if (!response?.ok) {
        setAuthenticated(false); setIsAdmin(false); setMembershipTier("standard"); setName("Kullanıcı"); setEmail(""); setReady(true); return;
      }
  ...
  async function logout() {
    ...
    setOpen(false); setAuthenticated(false); setIsAdmin(false);
    ...
    <header className="site-header shell-wide shared-header">
      ...
      {ready ? (
        <nav className="site-nav" aria-label="Ana menü">
          {isAdmin ? <Link href="/admin" className={pathname.startsWith("/admin") ? "active" : ""}>Yönetim merkezi</Link> : (
            <>
      ...
      <div className="header-actions">
        ...
        {ready && !isAdmin && (authenticated ? <Link href="/onboarding" className="button button-primary button-small header-appointment-button">Yeni hesaplama</Link> : <Link href="/appointment" className="button button-primary button-small header-appointment-button">Randevu al</Link>)}
      </div>
    </header>
  );
}
